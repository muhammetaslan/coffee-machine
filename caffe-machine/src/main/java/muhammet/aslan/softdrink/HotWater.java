...
public class HotWater extends Drink {
    ...
    public void prepare(Map<String, Integer> stockMeterialMap) throws SQLException {
        ...
        fillHotWater(ingredients.get(0).getName(), (stockMeterialMap.get(ingredients.get(0).getName()) - ingredients.get(0).getQuantity()));
    }

    @Override
    public void giveOrderInfo() {
        System.out.println("********************************************************");
        System.out.println("HotWater seçtiniz. Bu içeceğimizin içeriği şu şekildedir\n");
        List<Ingredient> ingredients = getRecipe().getIngredients();
